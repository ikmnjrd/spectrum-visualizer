...
// 各オブジェクト
let audioCtx;          // (共通)
let audioSourceNode;   // (共通)
let analyserNode_wave; // 波形データ
let analyserNode_sp;   // 周波数スペクトル
let tracks;            // トラック
...
// 周波数スペクトル用
let sp_width = 700;   // ※変更可能
let sp_height = 255;
let sp_padding_top = 15;
let sp_padding_left = 50;
let sp_padding_bottom = 25;

let sp_fftSize = 2048;  // fftのサイズ ※変更可能
let sp_fs;              // 周波数分解能(Frequency resolution)
let sp_440Hz;           // 440Hzの横幅
let sp_8192Hz_fftSize;  // 8192Hzのfftの最大サイズ

/**
 * window.onloadで読み込み初期化を行う。
 * @module init
 */
const init =() =>{
  canvas1 = document.getElementById('canvas_fq');
  ...
  audioEle = document.getElementById('audioController');
...
const draw = () => {
  ...
  ctx2.beginPath();
    ...
    for(let i=0; i < sp_8192Hz_fftSize; i++) {
  ...
  for(let i=0; i < 5; i++) {
    ctx2.fillText((shl* 440) + 'Hz',
                   sp_440Hz * _shl * (sp_width/sp_8192Hz_fftSize) + sp_padding_left - 17,
